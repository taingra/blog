#+TITLE: Minimal Debian 13 and \TeX Live Install
...
with a completely bare base-system, opting to even skip all graphical
desktops from my initial install.  I enjoy the process of starting
with essentially a bare server and only installing packages as I
actually need them.  Doing this process is interesting as it helps
highlight the applications I rely on, and in turn the software those
apps rely on.  In other words dependencies.
...
This tradeoff was highlighted for me when I went to install [[https://packages.debian.org/trixie/mpd][=mpd=]]
(Music Player Daemon).  =mpd= has a lot of dependencies and a few that
I found a little annoying.  I will likely never need to play Commodore
64 music files (=libsidplayfp6=), but that is also what makes =mpd=
great.  It is a flexible tool you can throw anything at.  Still I was
a little more annoyed to see JavaScript libraries installed just to
show the documentation pages (Sphinx).
...
though =mpd= is the only package I use that pulls these dependencies.
...
Of course this is not a real deal breaker for me.  I could build =mpd=
...
compiling packages.  I use Debian because I /like/ Debian.  I like the
philosophy, stability, and cooperative spirit of the project.  Debian
maintainers are making sane choices that makes stuff work with
reasonable defaults that supports a huge range of use cases.

With all that in mind, lets talk about LaTeX.

* Installing TeX Live on Debian

To build LaTeX documents we need to install the TeX Live distribution.
A full install of the texlive packages is huge and includes thousands
of TeX packages.  Majority of which I will never use and don't want to
install.  The problem is to compile Org-mode documents we need several
packages that are /only/ included in the extensive
=texlive-latex-extra= and =texlive-pictures= packages.

** Installing all of LaTeX the Normal Way
